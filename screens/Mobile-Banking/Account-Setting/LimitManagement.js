...
const LimitManagement = ({ navigation }) => {
  ...
  const fetchAccountData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/v1/customer/getAccount?accountNumber=${accountNumber}`, {
        headers: {
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#f9fafc]">
      {loading ? (
        <ActivityIndicator size="large" color={Color.PrimaryWebOrient} />
      ) : (
        <>
